...
const Sidebar = () => {
  ...
  const { openSearch } = usePlayer(); // Get the open function

  const NavItem = ({ to, icon: Icon, label, onClick }) => {
    const active = location.pathname === to;
    
    // Logic: If 'onClick' is provided, use it (div). Otherwise use Link.
    if (onClick) {
        return (
            <div 
                onClick={onClick}
                className="flex items-center gap-4 px-4 py-3 rounded-lg transition-all duration-200 text-gray-400 hover:text-white hover:bg-white/5 cursor-pointer"
            >
                <Icon size={22} />
    ...
    return (
      <Link 
        to={to} 
        className={`flex items-center gap-4 px-4 py-3 rounded-lg transition-all duration-200 ${active ? 'bg-purple-900/40 text-white border-l-4 border-purple-500' : 'text-gray-400 hover:text-white hover:bg-white/5'}`}
      >
        <Icon size={22} />
        <span className="font-semibold">{label}</span>
      </Link>
  ...
  return (
    <aside className="w-64 bg-black flex flex-col h-full border-r border-white/5 hidden md:flex">
      <div className="p-6">
      ...
      <div className="flex-1 px-2 space-y-1">
        ...
        {/* MODIFIED: Search triggers the modal function */}
        <NavItem 
            icon={Search} 
            label="Search" 
            onClick={openSearch} 
      ...
      {/* ... Premium Box (Keep existing code) ... */}
